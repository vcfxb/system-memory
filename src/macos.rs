...
use core::ptr;
use libc::{host_info64_t, host_statistics64, mach_host_self, mach_task_self, sysconf, sysctlbyname, vm_statistics64, HOST_VM_INFO64, HOST_VM_INFO64_COUNT, KERN_SUCCESS, _SC_PAGESIZE};
...
pub fn try_get_total_physical_memory() -> Result<u64, Errno> {
    ...
    let return_value = unsafe { 
        sysctlbyname(name.as_ptr(), &mut memsize as *mut _ as *mut c_void, &mut memsize_len, ptr::null_mut(), 0) 
    };
...
pub fn vm_statistics() -> Result<vm_statistics64, Errno> {
    ...
    let return_value = unsafe { host_statistics64(mach_port, HOST_VM_INFO64, &mut stats as *mut _ as host_info64_t, &mut count) };
